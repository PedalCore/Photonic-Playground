...
func _initialize() -> void:
	...
	var modulated = Solver.new(48, 40)
	modulated.configure([src])
	check(not modulated.set_source_amplitude(-1, 1.0) and not modulated.set_source_amplitude(0, NAN), "invalid source modulation is rejected")
	modulated.set_source_amplitude(0, 0.0)
	modulated.step(20)
	check(modulated.total_energy() == 0.0, "zero amplitude supplies no forcing")
	modulated.set_source_amplitude(0, -0.5)
	modulated.reset()
	modulated.step(160)
	var scaling_error := 0.0
	for i in range(one.field.size()):
		scaling_error = maxf(scaling_error, absf(modulated.field[i] + 0.5 * one.field[i]))
	check(scaling_error < 0.00001, "signed modulation scales the field and survives wave reset")
